...
public class Core implements MouseInputListener, MouseMotionListener, ComponentListener, WindowListener, WindowStateListener, ActionListener, Runnable
{
	...
	void unloadTitleScreen() //Выгрузка главного экрана
	{
		try
		{
			mainWindow.remove(titleButton1);
			mainWindow.remove(titleButton2);
			mainWindow.remove(titleButton3);
			mainWindow.remove(titleButton4);
			
			mainWindow.remove(titleMenu);
	...
	void unloadOptionsScreen() //Выгрузка экрана настроек
	{
		try
		{
		mainWindow.remove(setFullScreen);
		mainWindow.remove(optionsAccept);
		mainWindow.remove(backToMain);
		
		mainWindow.remove(optionsMenu);
